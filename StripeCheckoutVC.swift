...
class StripeCheckoutVC: UIViewController {
    ...
    @objc
    func pay() {
        guard let user = AppDelegate.shared().appUser, let userId = user.userId else { return }
        ...
        STPAPIClient.shared().createToken(withCard: cardParams) { token, error in
            ...
            parameters["paid_amount"] = "5.0"
            parameters["payment_info"] = tokenID
            parameters["payment_method"] = "Stripe"
            ApiManager.storePaymentInfo(parameters: parameters) { (response) in
